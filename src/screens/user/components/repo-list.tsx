...
function RepoList({ repos, filter }: RepoListProps) {
  const matchingRepos = matchSorter(repos, filter, {
    keys: [
      'name',
      { maxRanking: matchSorter.rankings.CASE_SENSITIVE_EQUAL, key: 'language' },
...
const ListItem = styled.li({ padding: '25px 0' }, ({ theme }) => theme.common.borderBottom);
...
function RepoListUserConsumer(props: any) {
  return (
