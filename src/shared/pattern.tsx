 ...
 */
function propStyles(styles: any) {
  return function dynamicStyles(props: any) {
 ...
 * applied.
 */
function variantStyles(styles: any) {
  return function dynamicStyles(props: any) {
    return Object.entries(props).map(([key, value]) => {
      if (styles[key]) {
        return applyStyles(styles[key][value as any], props);
      }
      return null;
    });
  };
...
function applyStyles(styles: any, props: any): any {
...
export const Text = styled.span(
  variantStyles({
    tint: {
      faded: ({ theme }: any) => ({ color: theme.colors.faded }),
      fadedExtra: ({ theme }: any) => ({ color: theme.colors.fadedExtra }),
    },
    size: {
...
export function IsolatedContainer({ children, ...props }: React.ComponentProps<any>) {
  return (
    <div
      css={{
        marginTop: 300,
...
export function LoadingMessagePage({ children }: React.ComponentProps<any>) {
  return (
    <IsolatedContainer>
